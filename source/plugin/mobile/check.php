 ...
 *
 *      $Id: check.php 31282 2012-08-03 02:30:10Z zhangjie $
 */
...
if(in_array('mobile', $_G['setting']['plugins']['available'])) {
	if(is_array($_GET['setting'])) {
		...
		$extendsetting = array();
		$query = DB::query('SELECT * FROM '.DB::table('mobile_setting').' WHERE skey IN (\'extend_used\', \'extend_lastupdate\')');
		while($mobilesetting = DB::fetch($query)) {
			$extendsetting[$mobilesetting['skey']] = $mobilesetting['svalue'];
		}
		$array = array(
